        ...
        return (
          <div key={step} className="flex items-center gap-2">
            <div className="flex items-center gap-1">
              <div
                className={`w-7 h-7 rounded-full flex shrink-0 items-center justify-center text-sm font-bold transition-all
                  ${isActive ? 'bg-pBrand text-white shadow-[0_0_10px_rgba(134,39,217,0.6)]' : isDone ? 'bg-pAccent text-navy' : 'bg-white/20 text-gray-300'}`}
              >
              ...
              <span
                className={`text-[13px] sm:text-sm font-bold tracking-tight whitespace-nowrap ${isActive ? 'text-pLight' : isDone ? 'text-pAccent' : 'text-gray-400'}`}
              >
                {label}
              </span>
            </div>
